...
The idea is to spilt the enumerated library into a set of smaller files containing 5000 molecules each.
...
def get_bb_data(aldehydes_file_path, acids_file_path, coupled_aa_file_path):
    ...
    with open(aldehydes_file_path, 'r') as f1:
        aldehydes = [line.split(' ')[0] for line in f1.read().splitlines()]
        aldehydes_names = [line.split(' ')[1] for line in f1.read().splitlines()]
    # Load carboxylic acid building blocks
    with open(acids_file_path, 'r') as f2:
        acids = [line.split(' ')[0] for line in f2.read().splitlines()]
        acids_names = [line.split(' ')[1] for line in f2.read().splitlines()]
    # Load coupled amino acid building blocks
    with open(coupled_aa_file_path, 'r') as f3:
        coupled_aa = [line.split(' ')[0] for line in f3.read().splitlines()]
        coupled_aa_names = [line.split(' ')[1] for line in f3.read().splitlines()]
...
def main():
    ...
    combinations = [
        (r1, r2, r3, n1, n2, n3, rxn)
        for n1, r1 in enumerate(coupled_aa_mols)
        for n2, r2 in enumerate(aldehyde_mols)
        for n3, r3 in enumerate(acid_mols)
    ]
