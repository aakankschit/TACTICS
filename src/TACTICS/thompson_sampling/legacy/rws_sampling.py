...
from .rws_reagent import Reagent
from .rws_utils import read_reagents
...
class RWSSampler:
    ...
    def evaluate(self, choice_list: List[int]) -> Tuple[str, float]:
        ...
        # For LookupEvaluator and DBEvaluator, skip molecule generation
        if isinstance(self.evaluator, (DBEvaluator, LookupEvaluator)):
            score = self.evaluator.evaluate(product_name)
            score = float(score)
            return [score, product_smiles, product_name]

        # For other evaluators, synthesize molecule
    ...
    def search(
        self,
        min_cpds_per_core: int = 30,
        percent_of_library: float = 0.001,
        stop: int = 1000,
        ...
        with tqdm(
            ...
            while len(uniq) < nsearch:
                ...
                if count % 100 == 0 and out_list:
                    ...
                    self.logger.info(
                        f"Iteration: {count} best score: {top_score:2f} smiles: {top_smiles} combi: {top_name}"
                    )
                # progressbar update
                incr = len(pairs_u)
