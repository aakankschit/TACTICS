...
def main():
    ...
    if summary_data:
        ...
        if detailed_results:
            ...
            axes[2].set_ylabel('Docking Score')
            axes[2].set_xlabel('')
            axes[2].set_xticklabels(axes[2].get_xticklabels(), rotation=15, ha='right')
            axes[2].grid(axis='y', alpha=0.3)

            plt.tight_layout()
            plt.savefig('rws_bayesucb_cats_comparison_fixed.png', dpi=300, bbox_inches='tight')
            print(f"Comparison plot saved to: rws_bayesucb_cats_comparison.png")
    else:
