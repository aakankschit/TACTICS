"""
Balanced Warmup Variance Comparison - ROCS Dataset

Compares per-reagent variance vs global variance using the new BalancedWarmup strategy
on a 3-component Quinazoline library with ROCS similarity scoring.
...
Key tests:
- Does per-reagent variance improve recovery on ROCS data?
- Does BalancedWarmup improve reproducibility (lower variance across runs)?
- How does the 3-component library (vs 2-component) affect results?

Note: ROCS scoring is MAXIMIZE mode (higher similarity = better)

Usage:
    python balanced_warmup_variance_comparison_rocs.py --output_dir /path/to/output
    python balanced_warmup_variance_comparison_rocs.py -o ./results/rocs_run_001
"""
...
NUM_TS_ITERATIONS = 18500  # Match paper settings
BATCH_SIZE = 100
NUM_REPLICATES = 5  # More replicates to measure variance reduction

# Strategy configurations - MAXIMIZE mode for ROCS
RWS_CONFIG = {
    "alpha": 0.1,
    "beta": 0.05,
...
def load_reference_data():
    ...
    # Sort in DESCENDING order (maximize ROCS) and get top 100
    ...
    print(f"  Best ROCS score: {ref_df['score'].max():.3f}")
...
def calculate_recovery_metrics(results_df, top_100_ref, maximize=True):
    ...
    # Best score found (max for ROCS)
    best_score = results_df['score'].max() if maximize else results_df['score'].min()
...
def combine_results(warmup_results, search_results, maximize=True):
    ...
    # Remove duplicates (keep best score - highest for ROCS)
    if maximize:
        combined_pd = combined_pd.sort_values('score', ascending=False).drop_duplicates(subset=['Name'], keep='first')
    else:
...
def print_statistical_summary(stat_results: dict, output_path: Path = None):
    ...
    if output_path is not None:
        ...
        with open(stats_file, 'w') as f:
            f.write("STATISTICAL ANALYSIS - Balanced Warmup Variance Comparison (ROCS)\n")
            f.write("=" * 80 + "\n\n")
...
def run_balanced_per_reagent_variance(replicate_num, ref_df, top_100_ref, seed=None):
    ...
    # Create strategy - MAXIMIZE for ROCS
    ...
    # Combine results (maximize=True for ROCS)
    results_df = combine_results(warmup_results, search_results, maximize=True)

    # Calculate metrics
    metrics = calculate_recovery_metrics(results_df, top_100_ref, maximize=True)
    ...
    print(f"    Best ROCS score: {metrics['best_score']:.3f}")
...
def run_legacy_rws(replicate_num, ref_df, top_100_ref):
    ...
    # Create legacy sampler (scaling=1 for maximize)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Compare per-reagent vs global variance in BalancedWarmup strategy (ROCS dataset)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python balanced_warmup_variance_comparison_rocs.py --output_dir ./results/rocs_run_001
    python balanced_warmup_variance_comparison_rocs.py -o ./results/$(date +%Y%m%d_%H%M%S)
        """
    )
    parser.add_argument(
        "-o", "--output_dir",
...
def main(output_dir: str = None):
    ...
    if output_dir is None:
        ...
        output_path = Path(f"./balanced_warmup_rocs_{timestamp}")
    else:
    ...
    print("="*80)
    print("Balanced Warmup Variance Comparison - ROCS Dataset")
    print("="*80)
    print(f"\nOutput directory: {output_path.absolute()}")
    print(f"\nDataset: 3-component Quinazoline library (ROCS similarity)")
    print(f"Mode: MAXIMIZE (higher ROCS score = better)")
    print(f"\nKey parameters:")
    ...
    if summary_data:
        ...
        # Compare with EnhancedWarmup baseline
        enhanced_recovery = [m['recovery_rate'] for m in all_metrics.get('Enhanced (Baseline)', [])]
        if per_reagent_recovery and enhanced_recovery:
            en_mean, en_std = np.mean(enhanced_recovery), np.std(enhanced_recovery)
            print(f"\n  Comparison with EnhancedWarmup baseline:")
            print(f"    Balanced + Per-Reagent: {pr_mean:.1%} +/- {pr_std:.1%}")
            print(f"    Enhanced (Baseline):    {en_mean:.1%} +/- {en_std:.1%}")
            print(f"    Improvement:            {(pr_mean - en_mean)*100:+.1f}%")
        ...
        if detailed_results:
            ...
            detailed_csv = output_path / 'balanced_warmup_variance_rocs_detailed.csv'
            detailed_df.to_csv(detailed_csv, index=False)
            print(f"\nDetailed results saved to: {detailed_csv}")

            summary_csv = output_path / 'balanced_warmup_variance_rocs_summary.csv'
            ...
            axes[0, 0].set_title('Top 100 Recovery Rate (ROCS)', fontsize=12, fontweight='bold')
            ...
            axes[0, 1].set_title('Top 10 Recovery Rate (ROCS)', fontsize=12, fontweight='bold')
            ...
            axes[1, 0].set_title('Best ROCS Score Found', fontsize=12, fontweight='bold')
            axes[1, 0].set_ylabel('ROCS Score')
            axes[1, 0].set_xlabel('')
            ...
            plt.suptitle('Balanced Warmup Variance Comparison - ROCS Dataset\n(3-component Quinazoline library)',
                        fontsize=14, fontweight='bold', y=1.02)
            plt.tight_layout()
            comparison_pdf = output_path / 'balanced_warmup_variance_rocs_comparison.pdf'
            ...
            if all_variance_dfs['Balanced + Per-Reagent Var']:
                ...
                if len(var_df_pr) > 0:
                    ...
                    axes2[0].set_title('Per-Reagent Variance Distribution (ROCS)', fontsize=12, fontweight='bold')
                    axes2[0].set_xlabel('Variance')
                    axes2[0].axvline(var_df_pr['variance'].mean(), color='r', linestyle='--',
                                    label=f'Mean: {var_df_pr["variance"].mean():.4f}')
                    axes2[0].legend()
                ...
                if len(var_df_gl) > 0:
                    ...
                    axes2[1].set_title('Global Variance Distribution (ROCS)', fontsize=12, fontweight='bold')
                    axes2[1].set_xlabel('Variance')
                    axes2[1].axvline(var_df_gl['variance'].mean(), color='r', linestyle='--',
                                    label=f'Mean: {var_df_gl["variance"].mean():.4f}')
                    axes2[1].legend()

                plt.tight_layout()
                distribution_pdf = output_path / 'balanced_warmup_variance_rocs_distribution.pdf'
                plt.savefig(distribution_pdf, dpi=300, bbox_inches='tight')
                plt.close(fig2)
                print(f"Variance distribution plot saved to: {distribution_pdf}")

                # ============================================================
                # Per-component variance analysis (3 components)
                # ============================================================
                print("\nGenerating per-component variance analysis...")

                fig3, axes3 = plt.subplots(1, 3, figsize=(15, 4))

                for comp_idx in range(3):
                    comp_data = var_df_pr[var_df_pr['component'] == comp_idx]
                    if len(comp_data) > 0:
                        sns.histplot(data=comp_data, x='variance', ax=axes3[comp_idx], bins=30)
                        axes3[comp_idx].set_title(f'Component {comp_idx} Variance\n(n={len(comp_data)} reagents)',
                                                 fontsize=11, fontweight='bold')
                        axes3[comp_idx].set_xlabel('Variance')
                        axes3[comp_idx].axvline(comp_data['variance'].mean(), color='r', linestyle='--',
                                               label=f'Mean: {comp_data["variance"].mean():.4f}')
                        axes3[comp_idx].legend()

                plt.suptitle('Per-Component Variance Distribution (3-component library)',
                            fontsize=12, fontweight='bold')
                plt.tight_layout()
                per_component_pdf = output_path / 'balanced_warmup_variance_rocs_per_component.pdf'
                plt.savefig(per_component_pdf, dpi=300, bbox_inches='tight')
                plt.close(fig3)
                print(f"Per-component plot saved to: {per_component_pdf}")
